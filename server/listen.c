                                  ...
                                  void *slave)
{
    ...
    // socklen_t len;
    // len = strlen(buf);
    const char *val[20][256];
    if (count_cca_execution == 0) {
        const char* env_value = getenv("APACHE_CCA_PORTS");
        printf("APACHE_CCA_PORTS  = %s\n",(env_value!=NULL)? env_value : "getenv returned NULL");
        char new_env_value[256];
        strcpy(new_env_value, env_value);
        char * token = strtok(new_env_value, "-");
        int i = 0;
        while( token != NULL ) {
            printf( " Token 1 %s\n", token ); //printing each token
            strcpy(val[i], token);
            token = strtok(NULL, "-");
            // printf( " Token 1 %s\n", val[i] ); //printing each token
            i++;
            // char *copy = (char *)malloc(strlen(token) + 1);
            // strcpy(copy, token);
            // char * key_pair = strtok(copy, ":");
            // printf( " Token 2 %s\n", token ); //printing each token
            // kv[i].key = key_val;
            // kv[i].value = key_val;
            // printf("Key pair %s\n", kv[i].value);
        ...
        int b = 0;
        // while(val[b] != '\0'){
        while(strlen(val[b]) > 0) {
            printf( " Copy Token val 1 %s\n", val[b] );
            char * key_pair = strtok(val[b], ":");
            int y = 0;
            while (y < 2) {
                if (y == 0){
                    // printf("set key %s", key_pair);
                    printf("set key %d",  atoi(key_pair));
                    kv[b].key = atoi(key_pair);
                }
                if (y == 1) {
                    printf("set value %s", key_pair);
                    kv[b].value = key_pair;
                }
                key_pair = strtok(NULL, ":");
                y++;
            }
            printf("Key pair %d %s\n", kv[b].key, kv[b].value);
            b++;
    ...
    while (sa) {
        ...
        //Pick the correct CCA based on the port
        int c = 0;
        // int length_of_cca;
        int cca_key;
        char cca_value[256];
        // printf("Pick cca : Key pair %d %s\n", kv[c].key, kv[c].value);
        while(strlen(kv[c].value) > 0) {
            // printf("ok1\n");
            if (current_port == kv[c].key) {
                // printf("ok2\n");
                // length_of_cca = strlen(kv[c].value);
                // printf("port matches\n");
                cca_key = kv[c].key;
                strcpy(cca_value, kv[c].value);
                break;
            }
            // printf("port does not match next>\n");
            c++;
        }
        
        // printf("before socket set\n");
        socklen_t len = strlen(cca_value);
        // printf("After socket length\n");

        int thesock;
        apr_os_sock_get(&thesock, new->sd);                            
        if (setsockopt(thesock, IPPROTO_TCP, TCP_CONGESTION, cca_value, len) != 0)
        {
            perror("setsockopt");
            printf("  [%s] ==\n", "setsockopt");
            return -1;
        }
        len = sizeof(cca_value);
        if (getsockopt(thesock, IPPROTO_TCP, TCP_CONGESTION, cca_value, &len) != 0)
        {
            perror("getsockopt");
            printf("  [%s] ==\n", "getsockopt");
            return -1;
        }
        printf("New CCA: %s for port %d\n", cca_value, current_port);
