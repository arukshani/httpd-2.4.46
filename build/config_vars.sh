...
if test -f ${DESTDIR}/mydata/loc/bin/apr-1-config; then
   APR_CONFIG=${DESTDIR}/mydata/loc/bin/apr-1-config
   APU_CONFIG=${DESTDIR}/mydata/loc/bin/apu-1-config
else
...
test -n "/mydata/loc/bin/apu-1-config" && APU_INCLUDEDIR="`${APU_CONFIG} --includedir`"

installbuilddir="/mydata/loc/build"
