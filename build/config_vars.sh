...
if test -f ${DESTDIR}/opt/loc/bin/apr-1-config; then
   APR_CONFIG=${DESTDIR}/opt/loc/bin/apr-1-config
   APU_CONFIG=${DESTDIR}/opt/loc/bin/apu-1-config
else
...
test -n "/opt/loc/bin/apu-1-config" && APU_INCLUDEDIR="`${APU_CONFIG} --includedir`"

installbuilddir="/opt/loc/build"
